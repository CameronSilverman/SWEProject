...
const DifficultyBadge: React.FC<{
	difficulty: IssueCardProps['difficulty'];
}> = ({ difficulty }) => {
	const colors = {
	...
	return (
		<span
			className={`px-2 py-1 rounded-full text-sm font-medium ${colors[difficulty]}`}
		>
			{difficulty}
		</span>
	);
		...
		<div className="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200 h-56 transition-transform duration-200 ease-in-out hover:scale-[1.02] hover:shadow-lg">
			<div className="p-4 h-full flex flex-col">
				...
				<div className="flex justify-between items-center mb-3">
					<div className="text-lg font-semibold text-gray-900 truncate max-w-[60%]">
						{title}
					</div>
